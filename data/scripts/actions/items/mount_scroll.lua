...
local function getRandomMount(player)
	...
	-- Verifica apenas as mounts que existem na tabela mountNames
	for mountId, mountName in pairs(mountNames) do
		local hasMount = player:hasMount(mountId)
	...
	local selectedMount = availableMounts[math.random(1, #availableMounts)]
	return selectedMount
end

local mountScroll = Action()

function mountScroll.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local mount = getRandomMount(player)
	...
	-- Show message with mount name
	if mount.name then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have received the " .. mount.name .. " mount!")
	else
...
mountScroll:id(48481)
mountScroll:register()
