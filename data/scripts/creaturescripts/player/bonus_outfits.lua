...
function addBonus.onLogin(player)
    ...
    local txt = {}
    for _, p in ipairs(params) do table.insert(txt, paramName[p] or "skill") end
    table.insert(txt, "shielding")      -- mostra o extra na mensagem

    player:sendTextMessage(
        MESSAGE_EVENT_ADVANCE,
        string.format(
            "Voce possui %d outfits completos e recebeu +%d em %s.",
            fullAddons, bonus, table.concat(txt, "/")
        )
    )
    return true
end
